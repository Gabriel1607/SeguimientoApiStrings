...
public class Main extends PApplet {
	...
	private void initArray() {
			...
			if (frameCount == 12) {
				...
				lyricsList.add(
						new FallenWords(words.get(indice), (int) random(15, 560), (int) random(-300, -10), this, 1));
				indice++;
				frameCount = 0;
		...
		for (int i = 0; i < lyricsList.size(); i++) {
			if ((mouseX > lyricsList.get(i).getPosX() - 20) && mouseX < (lyricsList.get(i).getPosX() + 20)
				...
				if (chosenWordsList.get(chosenPosition).getLyric().compareTo(lyricsList.get(i).getLyric()) == 0) {
					lyricsList.remove(i);
					chosenWordsList.get(chosenPosition).changeColor(69, 212, 53);
	...
	private void mouseDrag() {
		for (int i = 0; i < chosenWordsList.size(); i++) {
			if (chosenWordsList.get(i).isMatched() == false && (mouseX > chosenWordsList.get(i).getPosX() - 25)
					&& mouseX < (chosenWordsList.get(i).getPosX() + 25)
